...
namespace Seb.Fluid.Simulation
{
    public class FluidSim : MonoBehaviour
    {
        ...
        public ComputeBuffer obstacleCentres { get; private set; }
        public ComputeBuffer obstacleSizes { get; private set; }
        public ComputeBuffer obstacleRotations { get; private set; }
        ...
        void Initialize()
        {
            ...
            List<ComputeBuffer> updatePosBuffers = new List<ComputeBuffer>
            {
                positionBuffer,
                velocityBuffer
            };
            ...
            {
                obstacleCentres = CreateStructuredBuffer<float3>(obstacle.Length);
                obstacleSizes = CreateStructuredBuffer<float3>(obstacle.Length);
                obstacleRotations = CreateStructuredBuffer<float4>(obstacle.Length);

                bufferNameLookup.Add(obstacleCentres, "obstacleCentres");
                bufferNameLookup.Add(obstacleSizes, "obstacleSizes");
                bufferNameLookup.Add(obstacleRotations, "obstacleRotations");

                updatePosBuffers.Add(obstacleCentres);
                updatePosBuffers.Add(obstacleSizes);
                updatePosBuffers.Add(obstacleRotations);

                obstacleCentres.SetData(obstacle.Select(go => go.transform.position).ToArray());
                obstacleSizes.SetData(obstacle.Select(go => go.transform.lossyScale).ToArray());
                obstacleRotations.SetData(obstacle.Select(go => go.transform.rotation).ToArray());

                compute.SetInt("numObstacles", obstacle.Length);
            }
            else
            {
            ...
            SetBuffers(compute, updatePositionsKernel, bufferNameLookup, updatePosBuffers.ToArray());
